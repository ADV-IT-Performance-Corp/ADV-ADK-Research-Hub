...
All commit messages must start with an approved prefix (like `feat:` or `fix:`). Automatic merge messages such as `Merge branch 'master'` will fail CI; reword them (e.g., `chore: merge master into feature-X`) or squash the commit. See [docs/contribution_guide.md](docs/contribution_guide.md#commit-messages) for details.
